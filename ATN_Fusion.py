...
def ATN_Fusion(inputs, sen_len, target, sen_len_tr, attention, keep_prob1, keep_prob2, _id='all'):
    ...
    att_s_1 = tf.squeeze(att_s_1)

    w_att = tf.get_variable(
        name='w_att_w',
        shape=[FLAGS.max_sentence_len, FLAGS.max_sentence_len],
    ...
    gate = tf.sigmoid(tf.matmul(att_s_1, w_att) + tf.matmul(attention, w_att))
    att = gate * att_s_1 + (1 - gate) * attention
    att_s_1 = tf.expand_dims(att, 1)
...
def main(_):
    ...
    prob, att_1_s = ATN_Fusion(inputs_s, sen_len, target, tar_len, attention1, keep_prob1,
                               keep_prob2, FLAGS.t1)
